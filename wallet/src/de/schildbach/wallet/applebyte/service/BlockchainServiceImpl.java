...
package de.schildbach.wallet.applebyte.service;
...
import com.google.applebyte.discovery.DnsDiscovery;
import com.google.applebyte.discovery.IrcDiscovery;
import com.google.applebyte.discovery.PeerDiscovery;
import com.google.applebyte.discovery.PeerDiscoveryException;
import com.google.applebyte.store.BlockStore;
import com.google.applebyte.store.BlockStoreException;

import com.google.applebyte.store.SPVBlockStore;
import de.schildbach.wallet.applebyte.Constants;
import de.schildbach.wallet.applebyte.WalletApplication;
import de.schildbach.wallet.applebyte.WalletBalanceWidgetProvider;
import de.schildbach.wallet.applebyte.ui.WalletActivity;
import de.schildbach.wallet.applebyte.util.ThrottelingWalletChangeListener;
...
public class BlockchainServiceImpl extends android.app.Service implements BlockchainService
{
	...
	private static final String TAG = "Applebyte"+BlockchainServiceImpl.class.getSimpleName();
	...
	private void notifyCoinsReceived(final Address from, final BigInteger amount)
	{
		...
		final int precision = Integer.parseInt(prefs.getString(Constants.PREFS_KEY_ABY_PRECISION, Integer.toString(Constants.ABY_PRECISION)));
		...
		private void check()
		{
			...
			if (hasEverything && peerGroup == null)
			{
                ...
                try {
                    ...
                    Log.e("Applebyte", "There's no method: " + e.getLocalizedMessage());
                    Log.e("Applebyte", "Applebytej issue.  We're going to ignore this for now and just try and return nicely.");
                    return;
                }
		...
		try
		{
            ...
            if (!blockChainFileExists) { // Starting from scratch
                try {
                    ...
                    Log.d("Applebyte", "Couldn't find checkpoints file; starting from genesis");
                }
            }
	...
	public void notifyWidgets()
	{
        ...
        try {
            ...
            Log.e("Applebyte", "App server appears dead - Runtime Exception when running getAppWidgetIds.  Returning..");
            return;
        }
